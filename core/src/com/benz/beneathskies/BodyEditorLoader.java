...
public class BodyEditorLoader {
	...
	public ArrayList<Fixture> getAndAttachFixture(Body body, String name, FixtureDef fd, float scale) {
		ArrayList<Fixture> fixtureArrayList = new ArrayList<Fixture>();
		RigidBodyModel rbModel = model.rigidBodies.get(name);
		if (rbModel == null) throw new RuntimeException("Name '" + name + "' was not found.");
		...
		for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
			...
			polygonShape.set(vertices);
			fd.shape = polygonShape;
			myFixture = body.createFixture(fd);
			fixtureArrayList.add(myFixture);
